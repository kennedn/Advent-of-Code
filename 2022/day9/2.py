...
for j,d in enumerate(deltas):
  for i in range(len(knots)):
    ...
    if abs(knots[i-1][0] - knots[i][0]) <= 1 and abs(knots[i-1][1] - knots[i][1]) <= 1 :
      continue
    if knots[i-1][0] == knots[i][0] + 2 and knots[i-1][1] == knots[i][1]:
      knots[i][0] += 1
    elif knots[i-1][0] == knots[i][0] - 2 and knots[i-1][1] == knots[i][1]:
      knots[i][0] -= 1
    elif knots[i-1][1] == knots[i][1] + 2 and knots[i-1][0] == knots[i][0]:
      knots[i][1] += 1
    elif knots[i-1][1] == knots[i][1] - 2 and knots[i-1][0] == knots[i][0]:
      knots[i][1] -= 1
    else:
